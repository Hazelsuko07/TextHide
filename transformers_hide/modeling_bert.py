...
class BertForPreTraining(BertPreTrainedModel):
    ...
    Examples::
        ...
        >>> from transformers_hide import BertTokenizer, BertForPreTraining
        >>> import torch
...
class BertLMHeadModel(BertPreTrainedModel):
    ...
    Example::
        ...
        >>> from transformers_hide import BertTokenizer, BertLMHeadModel, BertConfig
        >>> import torch
...
class BertForNextSentencePrediction(BertPreTrainedModel):
    ...
    Example::
        ...
        >>> from transformers_hide import BertTokenizer, BertForNextSentencePrediction
        >>> import torch
...
class BertForSequenceClassification(BertPreTrainedModel):
    ...
    def forward(
        ...
        # print("inputs shape", input_ids_pub.shape)
        ...
        if input_ids_pub is not None:
            ...
            outputs_pub = self.bert(
                input_ids_pub,
                # attention_mask=attention_mask_pub,
                attention_mask=attention_mask,      # TODO: see if this works better
                token_type_ids=token_type_ids_pub,
                position_ids=position_ids,
            )
            pooled_output_pub = outputs_pub[1]

        # TextHide
        # print('labels', labels)
        if labels is not None:
            pooled_output, mix_labels, lams = mixup(
                pooled_output, labels, k=self.num_k, embeds_help=pooled_output_pub)
...
class BertClassificationHead(nn.Module):
    ...
    def __init__(self, config, small_cls=False):
        super().__init__()
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.out_proj = nn.Linear(config.hidden_size, config.num_labels)
        self.small_cls = small_cls

    def forward(self, features, **kwargs):
        # x = features[:, 0, :]  # take <s> token (equiv. to [CLS])
        x = features  # already taken the token in forward()
        # print(x.shape)
        x = self.dropout(x)
        x = self.dense(x)
        x = torch.tanh(x)
