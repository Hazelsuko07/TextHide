...
class Trainer:
    ...
    def __init__(
        ...
        train_dataset_pub: Optional[Dataset] = None,        # for Inter-TextHide
        ...
        **kwargs,
        ...
        self.train_dataset_pub = train_dataset_pub      # for Inter-TextHide
    ...
    def _get_train_sampler(self, pub=False) -> Optional[torch.utils.data.sampler.Sampler]:
        if pub is not True:
            target_dataset = self.train_dataset
        else:
            target_dataset = self.train_dataset_pub
        if isinstance(target_dataset, torch.utils.data.IterableDataset):
            return None
        elif is_torch_tpu_available():
            return get_tpu_sampler(target_dataset)
        else:
            return (
                RandomSampler(target_dataset)
                if self.args.local_rank == -1
                else DistributedSampler(target_dataset)
            )

    def get_train_dataloader(self, pub=False) -> DataLoader:
        """
        ...
        else: # get dataloader for the public dataset for Inter-TextHide
            if self.train_dataset_pub is None:
                raise ValueError("Trainer: training with Inter-TextHide requires a train_dataset.")
            train_sampler_pub = self._get_train_sampler(pub=True)

            return DataLoader(
                self.train_dataset_pub,
                batch_size=self.args.train_batch_size,
                sampler=train_sampler_pub,
                collate_fn=self.data_collator,
                drop_last=self.args.dataloader_drop_last,
            )
    ...
    def train(self, model_path: Optional[str] = None):
        ...
        if self.train_dataset_pub is not None:      # Inter-TextHide
            train_dataloader_pub = self.get_train_dataloader(pub=True)
        ...
        if self.train_dataset_pub is not None:
            logger.info("  Num pub examples = %d", self.num_examples(train_dataloader_pub))
        ...
        for epoch in train_iterator:
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                inputs_pub = next(iter(train_dataloader_pub)) if (self.train_dataset_pub is not None) else None
                    
                if steps_trained_in_current_epoch > 0:
                    steps_trained_in_current_epoch -= 1
                    continue
                
                # TODO: pass inputs_pub into training step
                tr_loss += self.training_step(model, inputs, inputs_pub = inputs_pub)
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                    ...
                    if (self.args.logging_steps > 0 and self.global_step % self.args.logging_steps == 0) or (
                        ...
                        epoch_iterator.set_description('Loss: {:.6f}'.format(logs["loss"]))
    ...
    def _prepare_inputs(
        self, inputs: Dict[str, Union[torch.Tensor, Any]], inputs_pub: Dict[str, Union[torch.Tensor, Any]], model: nn.Module
        ...
        if inputs_pub is not None:
            for k, v in inputs_pub.items():
                if isinstance(v, torch.Tensor):
                    inputs[k+'_pub'] = v.to(self.args.device)
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]], inputs_pub: Dict[str, Union[torch.Tensor, Any]]) -> float:
        """
        ...
        model.train()
        inputs = self._prepare_inputs(inputs, inputs_pub, model)
        # print(inputs)
    ...
    def prediction_step(
        ...
        inputs = self._prepare_inputs(inputs, None, model)

        with torch.no_grad():
            if self.search:
                outputs = model(**inputs, search=True)
                return (None, outputs.detach(), None)
            else:
                outputs = model(**inputs)
            if has_labels:
